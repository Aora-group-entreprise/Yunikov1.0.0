...
import { createStory as createRemoteStory, listActiveStories, markStoryView, uploadStoryMedia } from "./features/stories/service";
...
type StoreContextValue = {
  ...
  addPost: (input: { caption: string; image?: string; mediaFile?: File; location?: string; hashtags?: string[] }) => Promise<boolean>;
  addStory: (input: { caption: string; image?: string }) => Promise<boolean>;
...
function StoreProvider({ children }: { children: ReactNode }) {
  ...
  const hydrate=useCallback(async(authId:string)=>{
    ...
    if(storiesResult.data){ setState(prev=>({...prev,stories:storiesResult.data.map((story:any)=>({id:String(story.id),user:activeUserRef.current,image:story.media_url,viewed:false}))})); }
    if(feedResult.data){
      const posts:DemoPost[]=feedResult.data.map(post=>({id:String(post.id),user:post.author?{id:post.author.id,username:post.author.username,displayName:post.author.display_name,avatar:post.author.avatar_url??"",bio:"",followers:0,following:0,posts:0}:activeUserRef.current,image:post.media_url??IMG.neon,caption:post.caption,hashtags:Array.isArray(post.hashtags)?post.hashtags:[],likes:post.likes??0,comments:post.comments??0,shares:post.shares??0,views:post.views??0,location:post.location??undefined}));
      setPeople(prev=>{const next=prev.map(user=>user.id==="me"?{...user,...activeUserRef.current}:user); for(const post of posts){if(post.user.id!==activeUserRef.current.id&&!next.some(u=>u.id===post.user.id))next.push(post.user);} return next;});
  ...
  useEffect(()=>{
    if(!supabase)return;
    let mounted=true;
    void supabase.auth.getSession().then(({data})=>{const id=data.session?.user.id;if(mounted&&id){void hydrate(id);}});
    const {data:listener}=supabase.auth.onAuthStateChange((event,session)=>{
      if(event==="SIGNED_OUT"){setRemoteUserId(null);}
      else if(session?.user.id){setRemoteUserId(session.user.id);void hydrate(session.user.id);}
    });
  ...
  const addPost=useCallback(async(input:{caption:string;image?:string;mediaFile?:File;location?:string;hashtags?:string[]}):Promise<boolean>=>{
    if(!remoteUserId){showToast("Sign in required");return false;}
  ...
  const addStory=useCallback(async(input:{caption:string;image?:string}):Promise<boolean>=>{
    if(!remoteUserId){showToast("Sign in required");return false;}
    if(!input.image){showToast("Story media required");return false;}
    try{
      const response=await fetch(input.image);
      const blob=await response.blob();
      const file=new File([blob],"story-media",{type:blob.type||"image/jpeg"});
      const uploaded=await uploadStoryMedia(file);
      if(uploaded.error||!uploaded.path)throw uploaded.error||new Error("Story upload failed");
      const created=await createRemoteStory(uploaded.path,file.type.startsWith("video/")?"video":"image",input.caption);
      if(created.error)throw created.error;
      const stories=await listActiveStories();
      if(stories.data)setState(prev=>({...prev,stories:stories.data.map((story:any)=>({id:String(story.id),user:activeUserRef.current,image:story.media_url,viewed:false}))}));
      showToast("Story shared");
      return true;
    }catch(error:any){showToast(error?.message||"Story failed");return false;}
  },[remoteUserId,showToast]);
...
function PostCard({ post }: { post: DemoPost }) {
  const [,navigate]=useLocation(); const {state,people,toggleLike,toggleSave,addComment,toggleFollow,showToast}=useDemo();
...
function Create() {
  ...
  const publish = async () => {
    ...
    if (tab === "story" ? !media : !caption.trim() && !media) { showToast(tab === "story" ? "Add a photo or video first" : "Add a photo or caption first"); return; }
    setPublishing(true);
    const success = tab === "story"
      ? await addStory({ caption, image: media || undefined })
      : await addPost({ caption, image: media || undefined, mediaFile: mediaFile || undefined, location, hashtags: hashtags.split(/[,\s]+/).map((tag) => tag.trim().replace(/^#?/, "#")).filter((tag) => tag.length > 1) });
    setPublishing(false);
...
function StoryPage() {
  ...
  useEffect(() => {
    if (story?.id && /^\d+$/.test(story.id)) void markStoryView(Number(story.id));
  }, [story?.id]);
  if (!story) return <PageShell nav={false}><TopBar title="Story" /><EmptyState icon={<ImageIcon size={28} />} title="Story unavailable" body="This story may have expired." action={<button onClick={() => navigate("/")} className="mt-5 text-pink-400 text-sm">Back to feed</button>} /></PageShell>;
...
function Chat() {
  ...
  const { people, sendMessage, showToast } = useDemo();
  const contact = people.find((person) => person.id === params.userId);
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        const conversation = await createDirectConversation(contact.id);
  ...
  if (!contact) return <PageShell nav={false}><TopBar title="Conversation" /><EmptyState icon={<MessageCircle size={28} />} title="User unavailable" body="This conversation is no longer available." action={<button onClick={() => navigate("/messages")} className="mt-5 text-pink-400 text-sm">Back to messages</button>} /></PageShell>;
...
function CallPage() {
  ...
  if (!contact) return <PageShell nav={false}><TopBar title="Call" /><EmptyState icon={<Phone size={28} />} title="User unavailable" body="This call cannot be started." action={<button onClick={() => navigate("/")} className="mt-5 text-pink-400 text-sm">Back to feed</button>} /></PageShell>;
...
function PostDetail() { const params = useParams<{ postId?: string }>(); const { state } = useDemo(); const post = state.posts.find((item) => item.id === params.postId) ?? state.posts[0]; if (!post) return <PageShell><TopBar title="Post" /><EmptyState icon={<ImageIcon size={28} />} title="Post unavailable" body="This post may have been deleted or is not in your feed." /></PageShell>; return <PageShell><TopBar title="Post" /><div className="max-w-2xl mx-auto px-3 pt-3"><div className="aspect-[4/5] rounded-2xl overflow-hidden"><PostCard post={post} /></div></div></PageShell>; }
function FollowersPage({ mode }: { mode: "followers" | "following" }) { const [, navigate] = useLocation(); const { state, people, getUser } = useDemo(); const params = useParams<{ userId?: string }>(); const user = getUser(params.userId); const list = people.filter((person) => person.id !== user.id && (mode === "following" ? state.following.includes(person.id) || person.id === "1" : true)); return <PageShell><TopBar title={mode === "followers" ? "Followers" : "Following"} /><div className="px-4 py-4"><p className="text-white/45 text-sm mb-4">{user.displayName} · {list.length} people</p>{list.map((person) => <button key={person.id} onClick={() => navigate(`/user/${person.id}`)} className="w-full flex items-center gap-3 py-3 text-left"><Avatar user={person} size="md" ring /><div><p className="font-semibold text-sm">{person.displayName}</p><p className="text-white/45 text-xs">@{person.username}</p></div><ChevronRight size={16} className="ml-auto text-white/25" /></button>)}</div></PageShell>; }
...
function SettingsDetail({ kind }: { kind: "privacy" | "security" | "storage" | "account" | "blocked" | "verify" }) {
  const [, navigate] = useLocation();
   const { state, activeUser, people, updateState, toggleBlocked, showToast } = useDemo();
  const titles = { privacy: "Privacy", security: "Security", storage: "Storage", account: "Username", blocked: "Blocked users", verify: "Verify account" };
  if (kind === "account") return <PageShell><TopBar title={titles[kind]} /><form className="max-w-xl mx-auto px-4 py-6 space-y-4" onSubmit={(event) => { event.preventDefault(); showToast("Account details saved"); }}><LabeledInput label="Username" value={activeUser.username} onChange={(value) => updateState({ profile: { ...state.profile, username: value } })} /><button className="w-full py-3.5 rounded-2xl font-semibold" style={{ background: GRADIENT }}>Save account</button></form></PageShell>;
  if (kind === "storage") return <PageShell><TopBar title={titles[kind]} /><div className="px-4 py-6 max-w-xl mx-auto"><div className="rounded-2xl bg-white/[.04] border border-white/[.07] p-5 mb-5"><p className="text-white/45 text-xs uppercase tracking-wider">Storage</p><p className="text-3xl font-bold mt-2">{Math.max(1, state.posts.length * 4)} MB</p><p className="text-white/45 text-sm mt-1">Saved posts, messages and preferences on this device.</p></div><button onClick={() => { localStorage.removeItem("yuniko-media-cache"); showToast("Cached media cleared"); }} className="w-full py-3.5 rounded-2xl bg-red-500/10 border border-red-500/20 text-red-400 font-semibold">Clear cached media</button></div></PageShell>;
