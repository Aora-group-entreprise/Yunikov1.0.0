...
function StoreProvider({ children }: { children: ReactNode }) {
  ...
  const value:StoreContextValue={state,activeUser,getUser:id=>id==="me"||!id?activeUser:people.find(p=>p.id===id)??people[1],toast,showToast,toggleLike:id=>toggleEngagement(id,"liked"),toggleSave:id=>toggleEngagement(id,"saved"),toggleFollow:id=>{void toggleFollowRemote(id);},addComment:(id,comment)=>{void addComment(id,comment);},addStoryReply:(id,reply)=>setState(prev=>({...prev,storyReplies:{...prev.storyReplies,[id]:[...(prev.storyReplies[id]||[]),reply]}})),sendMessage:(id,message)=>setState(prev=>({...prev,messages:{...prev.messages,[id]:[...(prev.messages[id]||[]),message]}})),archiveConversation:id=>setState(prev=>({...prev,archived:prev.archived.includes(id)?prev.archived.filter(x=>x!==id):[...prev.archived,id]})),addPost,addStory:input=>{setState(prev=>({...prev,stories:[{id:`s-${Date.now()}`,user:activeUser,image:input.image||IMG.neon},...prev.stories]}));showToast("Story shared");},updateProfile:profile=>{void saveProfile(profile);},updateState:patch=>setState(prev=>({...prev,...patch})),toggleBlocked:id=>setState(prev=>({...prev,blocked:prev.blocked.includes(id)?prev.blocked.filter(x=>x!==id):[...prev.blocked,id]}))};
