...
export async function listNotifications(){
  ...
  return requireSupabase().from("notifications")
    .select("id,actor_id,type,post_id,story_id,message,group_key,count,read_at,created_at")
...
export async function getNotificationPreferences(){
  const userId=await currentLegacyUserId();
  return requireSupabase().from("notification_preferences")
    .select("user_id,in_app_enabled,push_enabled,digest_enabled,updated_at").eq("user_id",userId).maybeSingle();
}
export async function updateNotificationPreferences(input:{in_app_enabled?:boolean;push_enabled?:boolean;digest_enabled?:boolean}){
  const userId=await currentLegacyUserId();
  return requireSupabase().from("notification_preferences")
    .upsert({user_id:userId,...input,updated_at:new Date().toISOString()},{onConflict:"user_id"}).select().single();
}
